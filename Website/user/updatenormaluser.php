...
<body>
  ...
  <li><a href=\"comhomepage.php\">Communities</a></li>
        <li><a href="friendrequests.html">Friend Requests</a></li>
...
<form method="post">
    ...
    if(isset($_POST['u2'])){
      $db  = new mysqli('localhost','root','','team62');
             ...
             $mail=$_COOKIE['userID'];
            if(!empty($fn)){
              $qur=$db->query("Update Normal_Users set first_name = '{$fn}' where email = '{$mail}'");
              if ($qur) {
...
<form method="post">
    ...
    if(isset($_POST['u3'])){
      $db  = new mysqli('localhost','root','','team62');
             ...
             $mail=$_COOKIE['userID'];
            if(!empty($ln)){
              $qur=$db->query("Update Normal_Users set last_name = '{$ln}' where email = '{$mail}'");
              if ($qur) {
   /* echo "Incorrect username or password";*/
} else {
    echo "Error: " .' '. "<br>" . mysqli_error($db);
    ...
    if(isset($_POST['u5'])){
      $db  = new mysqli('localhost','root','','team62');
             ...
             $mail=$_COOKIE['userID'];
            if(!empty($dob)){
              $qur=$db->query("Update Normal_Users set date_of_birth = '{$dob}' where email = '{$mail}'");
              if ($qur) {
     /* echo "Incorrect username or password";*/
} else {
    echo "Error: " .' '. "<br>" . mysqli_error($db);
