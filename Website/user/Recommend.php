...
<body>
    <font color = "FF0000"><h1>Recommended Games</h1></font>
      <?php
        $mail=$_COOKIE['userID'];
        //$keyw=$_GET['id'];
...
function getUserType($mail){
  ...
  <div class="tfclear"></div>
   ...
   $db  = new mysqli('localhost','root','','team62');
    $mail=$_COOKIE['userID'];
    $qur=$db->query("select game_name, recommender_email
                      from games_recommendedby_normal_users
                      where reciever_email = '{$mail}'");
      if ($qur){
      while ($a1=$qur->fetch_assoc()){
        $game = $a1['game_name'];
        $n = $a1['recommender_email'];
        echo "<TR ALIGN=\"CENTER\">
      <TD><a href = \"game.php?id=$game\">$game</a></TD>
      <TD><a href = \"nuprofile.php?id=$n\">$n</a></TD>
   </TR>";
      }
    }
echo "</TABLE>";
